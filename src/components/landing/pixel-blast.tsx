  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const animate = () => {
        ...
        if (liquidEffect)
          (liquidEffect as Effect & { uniforms: Map<string, { value: number }> }).uniforms.get("uTime")!.value =
            uniforms.uTime.value;
        if (composer) {
          if (touch) touch.update();
          composer.passes.forEach((p) => {
            if (p instanceof EffectPass && 'effects' in p) {
              const effects = (p as unknown as { effects: Effect[] }).effects;
              if (effects)
                effects.forEach((eff: Effect & { uniforms?: Map<string, { value: number }> }) => {
      ...
      if (t.liquidEffect) {
        (t.liquidEffect as Effect & { uniforms: Map<string, { value: number }> }).uniforms.get("uStrength")!.value = liquidStrength;
        (t.liquidEffect as Effect & { uniforms: Map<string, { value: number }> }).uniforms.get("uFreq")!.value = liquidWobbleSpeed;
      }
    }
